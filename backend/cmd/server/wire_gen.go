...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	checkInService := service.NewCheckInService(checkInRepository, userRepository, billingCacheService, client, apiKeyAuthCacheInvalidator, configConfig)
	...
	imagePlaygroundHandler := handler.NewImagePlaygroundHandler(settingService)
	infiniteCanvasHandler := handler.NewInfiniteCanvasHandler(settingService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, checkInHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, imagePlaygroundHandler, infiniteCanvasHandler, idempotencyCoordinator, idempotencyCleanupService)
