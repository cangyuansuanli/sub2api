    ...
    >
      ...
      <div v-else-if="selectedOverview" class="space-y-4">
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          ...
          <OverviewStat :label="t('admin.affiliates.overview.availableQuota')" :value="formatCurrency(selectedOverview.available_quota)" />
          <OverviewStat :label="t('admin.affiliates.overview.historyQuota')" :value="formatCurrency(selectedOverview.history_quota)" />
        </div>
      </div>
    </BaseDialog>
  </AppLayout>
...
import {formatDateTime as formatDisplayDateTime, formatCurrency} from '@/utils/format'
...
const AmountText = defineComponent({
  ...
  setup(amountProps) {
    ...
    }, formatCurrency(amountProps.value))
  },
})
