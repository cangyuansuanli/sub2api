...
let marketDataJsonUrl = ''
let initialized = false

function bindRefs(scope) {
  refs = {
    statModels: scope.querySelector('#stat-models'),
    statPlatforms: scope.querySelector('#stat-platforms'),
    statGroups: scope.querySelector('#stat-groups'),
    statUnit: scope.querySelector('#stat-unit'),
    platformTabs: scope.querySelector('#platform-tabs'),
    groupTabs: scope.querySelector('#group-tabs'),
    search: scope.querySelector('#model-search'),
    modelSections: scope.querySelector('#model-sections'),
    emptyState: scope.querySelector('#empty-state'),
    updatedAt: scope.querySelector('#updated-at'),
    unitDescription: scope.querySelector('#unit-description'),
    modal: scope.querySelector('#model-modal'),
    modalClose: scope.querySelector('#modal-close'),
    modalContent: scope.querySelector('#modal-content'),
  }
}
...
async function fetchMarketDataJson() {
  if (!marketDataJsonUrl) {
    throw new Error('model market data URL not configured')
  }
  const response = await fetch(`${marketDataJsonUrl}?t=${Date.now()}`, {
    cache: 'no-store',
  })
...
function onEscapeKeydown(event) {
  if (event.key === 'Escape' && refs.modal && !refs.modal.hidden) closeModal()
}
...
export function initModelMarketPage(scope, options = {}) {
  if (initialized) return
  initialized = true

  marketDataJsonUrl = options.dataUrl || ''
  if (options.initialData) {
    applyMarketData(options.initialData)
  }
  ...
  bindEvents()
  loadMarketData()
  refreshTimer = window.setInterval(refreshMarketData, 30000)
}

export function destroyModelMarketPage() {
  if (!initialized) return
  initialized = false
  if (refreshTimer) {
